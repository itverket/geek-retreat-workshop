...
namespace WeatherUndergroundService.Clients
{
    public class CurrentWeather
    {
        ...
        /// <summary>
        ///     Get the current weather of a specific city by indicating the city, country, language and units (metric or
        ///     imperial).
        /// </summary>
        /// <param name="city">Name of the city.</param>
        /// <param name="country">Country of the city.</param>
        ...
        /// <returns> The weather information.</returns>
        public static SingleResult<CurrentWeatherResult> GetByCityName(string city, string country, string language,
            string units)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(city) || string.IsNullOrEmpty(country))
                    return new SingleResult<CurrentWeatherResult>(null, false, "City and/or Country cannot be empty.");
                var response =
                    ApiClient.GetResponse("/weather?q=" + city + "," + country + "&lang=" + language + "&units=" + units);
                return Deserializer.GetWeatherCurrent(response);
            }
            catch (Exception ex)
            {
        ...
        /// <param name="units">The units of the date (metric or imperial).</param>
        /// <returns> The weather information.</returns>
        public static SingleResult<CurrentWeatherResult> GetByCoordinates(double lat, double lon, string language,
            string units)
        {
            try
            {
                var o =
                    ApiClient.GetResponse("/weather?lat=" + lat + "&lon=" + lon + "&lang=" + language + "&units=" +
                                          units);
                return Deserializer.GetWeatherCurrent(o);
            }
            catch (Exception ex)
        ...
        /// <summary>
        ///     Get the current weather of a specific city by indicating its 'OpenwWeatherMap' identifier, language and units
        ///     (metric or imperial).
        /// </summary>
        /// <param name="id">City 'OpenwWeatherMap' identifier.</param>
        /// <param name="language">
